...
const providerGame = {
    em: $(".provider-minigame"),
    active: false,
    count: 0,

    onMessage(data) {
        if (data.job == "providergame")
            this.build();
    },
    ...
    build() {
        ...
        var maxBox = Math.floor(Math.random() * (8-5)) + 5;

        for (var i = 0; i < maxBox; i++) {
            var position = this.getSpawnPosition(this.em.height() / 2 - 350, 250, this.em.width() / 2 - 350, 750);
            this.em.append(`
                <div id="provider-box-${i}" class="box" style="left: ${position.x}px; top: ${position.y}px;"></div>
            `);


            var theBox = $("#provider-box-" + i);
            theBox.on("click", function() {
                $(this).remove();
                providerGame.count++;

                if (providerGame.count >= maxBox) {
                    providerGame.post("provider:gameDone", [true]);
                    providerGame.destroy();
                }
            })
        }
...
window.addEventListener("message", (event) => {
    providerGame.onMessage(event.data);
})
