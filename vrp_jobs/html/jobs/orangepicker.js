...
const orangepickerJob = new Vue({
    el: ".main-orangepicker",
    data: {
        active: false,
        zoom: 0,
        job: "",
...
const orangeGame = {
    em: $(".orangepicker-minigame"),
    spawn: $(".orangepicker-minigame .spawn-zone"),
    basket: $(".orangepicker-minigame .drag-zone"),
    active: false,
    count: 0,

    onMessage(data) {
        if (data.job == "orangegame")
            this.build();
    },
    ...
    build() {
        ...
        var maxOranges = Math.floor(Math.random() * 6) + 7;

        for (var i = 0; i < maxOranges; i++) {
            var position = this.getSpawnPosition(this.spawn.height() / 2 - 350, 250, this.spawn.width() / 2 - 350, 750);
            this.em.append(`
                <div id="orangepicker-orange-${i}" class="orange" style="left: ${position.x}px; top: ${position.y}px;"></div>
            `);


            var theOrange = $("#orangepicker-orange-" + i);
            theOrange.draggable({containment: 'window', scroll: false});
        }
        ...
        this.basket.droppable({
            drop: (event, ui) => {
                ...
                if (this.count >= maxOranges) {
                    this.post("orangepicker:gameDone");
                    this.destroy();
                }
            }
        })
...
window.addEventListener("message", (event) => {
    orangeGame.onMessage(event.data);
})
