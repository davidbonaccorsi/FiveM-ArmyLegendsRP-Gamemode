...
const weedGame = {
    em: $(".weed-minigame"),
    spawn: $(".weed-minigame .spawn-zone"),
    basket: $(".weed-minigame .drag-zone"),
    active: false,
    count: 0,

    onMessage(data) {
        if (data.job == "weedgame")
            this.build();
    },
    ...
    build() {
        ...
        for (var i = 0; i < maxLeafs; i++) {
            ...
            this.em.append(`
                <div id="weed-leaf-${i}" class="leaf" style="left: ${position.x}px; top: ${position.y}px;"></div>
            `);


            var theLeaf = $("#weed-leaf-" + i);
