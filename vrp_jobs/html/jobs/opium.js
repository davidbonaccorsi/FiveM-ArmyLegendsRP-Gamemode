...
const opiumGame = {
    em: $(".opium-minigame"),
    spawn: $(".opium-minigame .spawn-zone"),
    basket: $(".opium-minigame .drag-zone"),
    active: false,
    count: 0,

    onMessage(data) {
        if (data.job == "opiumgame")
            this.build();
    },
    ...
    build() {
        ...
        var maxSeeds = Math.floor(Math.random() * 6) + 7;

        for (var i = 0; i < maxSeeds; i++) {
            var position = this.getSpawnPosition(this.spawn.height() / 2 - 350, 250, this.spawn.width() / 2 - 350, 750);
            this.em.append(`
                <div id="opium-seed-${i}" class="seed" style="left: ${position.x}px; top: ${position.y}px;"></div>
            `);


            var theSeed = $("#opium-seed-" + i);
            theSeed.draggable({containment: 'window', scroll: false});
        }
        ...
        this.basket.droppable({
            drop: (event, ui) => {
                ...
                if (this.count >= maxSeeds) {
                    this.post("opium:gameDone");
                    this.destroy();
                }
            }
        })
...
window.addEventListener("message", (event) => {
    opiumGame.onMessage(event.data);
})
