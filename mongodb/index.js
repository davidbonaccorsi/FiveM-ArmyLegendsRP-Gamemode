...
async function dbFindAsync(params) {
    if (!checkDatabaseReady()) return;
    ...
    let documents = await collection.find(query, options).limit(params.limit || 100).toArray();
    return utils.exportDocuments(documents);
}

exports('findSync', dbFindAsync);

async function dbFindMultiple(params) {
    if (!checkDatabaseReady()) return;
    if (!checkParams(params)) return console.log(`[MongoDB][ERROR] exports.find: Invalid params object.`);
    ...
    let parseQuery = object => {
        let retval = {};
        if (object.query.length >= 1) retval['$or'] = [...object.query];
        if (object.required.length >= 1) retval['$and'] = [...object.required];
        return retval;
    }

    let documents = await collection.find(parseQuery(params), options).limit(params.limit || 100).toArray();
    
    return utils.exportDocuments(documents);
}

exports('findMultiple', dbFindMultiple);

async function dbCountAsync(params) {
    if (!checkDatabaseReady()) return;
...
exports("countSync", dbCountAsync);
...
async function dbAggregate(params, callback) {
    if (!checkDatabaseReady()) return;
    ...
    try {
        ...
        if(callback) utils.safeCallback(callback, true, [result[0]]);
