...
local function radioNameGetter_orig(source)
	return {name = GetPlayerName(source), user_id = vRP.getUserId({source})}
end
...
function addPlayerToRadio(source, radioChannel)
	if not canJoinChannel(source, radioChannel) then
		...
		return TriggerClientEvent('pma-voice:removePlayerFromRadio', source, source, user_id)
	end
	...
	local radioUsers = {}
	for player, _ in pairs(radioData[radioChannel]) do
		radioUsers[player] = radioNameGetter(player)
	end
	TriggerClientEvent('pma-voice:syncRadioData', source, radioData[radioChannel], GetConvarInt("voice_syncPlayerNames", 0) == 1 and plyName,radioUsers)
end
...
function removePlayerFromRadio(source, radioChannel)
	...
	for player, _ in pairs(radioData[radioChannel]) do
		TriggerClientEvent('pma-voice:removePlayerFromRadio', player, source, user_id)
	end
	...
	if radioTbl then
		...
		for player, _ in pairs(radioTbl) do
			-- if player ~= source then
				TriggerClientEvent('pma-voice:setTalkingOnRadio', player, source, talking, user_id)
