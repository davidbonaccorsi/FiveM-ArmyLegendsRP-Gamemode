...
function syncRadioData(radioTable, localPlyRadioName, radioUsers)
	...
	TriggerEvent("fp-radio:updateList",radioUsers)
end
...
function setTalkingOnRadio(plySource, enabled, user_id)
	toggleVoice(plySource, enabled, 'radio')
	radioData[plySource] = enabled
	-- print("wtf")
	TriggerEvent("fp-radio:setTalking",user_id,enabled)
...
function addPlayerToRadio(plySource, plyRadioName)
	...
	if GetConvarInt("voice_syncPlayerNames", 0) == 1 then
		...
		TriggerEvent("fp-radio:addUserToRadio",{
			serverid = plySource,
			name = plyRadioName.name,
			user_id = plyRadioName.user_id
		})
	end
...
function removePlayerFromRadio(plySource,user_id)
	if plySource == playerServerId then
		...
		radioNames = {}
		TriggerEvent("fp-radio:updateList",radioNames)
		radioData = {}
		...
		if GetConvarInt("voice_syncPlayerNames", 0) == 1 then
			...
			TriggerEvent("fp-radio:remUserToRadio",{
				serverid = user_id,
			})
		end
	end
