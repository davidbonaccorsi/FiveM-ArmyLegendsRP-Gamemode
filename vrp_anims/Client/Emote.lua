...
local PtfxThread = false
local PtfxWait = 500
local PtfxNoProp = false
local locked = false

exports("cancelEmotes", function()
...
RegisterKeyMapping("cancelEmote", "Anuleaza o animatie", "keyboard", "X")

RegisterCommand("openEmoteMenu", function()
  OpenEmoteMenu()
end)

RegisterKeyMapping("openEmoteMenu", "Meniu de animatii", "keyboard", "F6")
...
function EmoteCancel()
  ...
  if not exports.vrp:canStopAnim() then
    TriggerEvent("vrp-hud:notify", "Nu poti anula aceasta animatie.", "error")
    return
  end

  local falling = IsPedFalling(PlayerPedId()) or 0
  if ( (falling > 0) or IsPedInParachuteFreeFall(PlayerPedId()) ) then return end
...
function EmoteChatMessage(args)
  if args ~= display then
    TriggerEvent("chatMessage", "^5[Emotes]^7 "..args)
  end
end
...
function EmoteMenuStart(args, hard)
    if not exports.vrp:canStopAnim() then return end
