        ...
        if #args > 0 then
            ...
            if(distance ~= -1 and distance < 3) then
                if DP.Shared[emotename] ~= nil then
                    ...
                    TriggerServerEvent("ServerEmoteRequest", GetPlayerServerId(target), emotename, "", ((args[2] or ""):lower() == "verificarequestpaste"))

                    if (args[2] or ""):lower() ~= "verificarequestpaste" then
...
RegisterNetEvent("vrp-anims:denySharedEmote", function()
...
RegisterNetEvent("vrp-anims:acceptSharedEmote", function()
    if isRequestAnim then
        target, distance = GetClosestPlayer()
        if(distance ~= -1 and distance < 3) then
