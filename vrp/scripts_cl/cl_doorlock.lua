-- C: plesalex100, mai bine nu se putea face
...
local allDoors = {
    ...
    -- Bijuterie
    -- ["biju_1"] = {pos = vector3(-630.42651367188, -238.43754577637, 38.206531524658), model = 9467943},
    -- ["biju_2"] = {pos = vector3(-631.95538330078, -236.33326721191, 38.206531524658), model = 1425919976},
}

RegisterCommand("createdoors", function()
	local doors = true
	RegisterCommand("stopdoors", function()
		doors = false
	end)
	...
	while doors do
		...
		if IsPedShooting(ped) then
			local exist, obj = GetEntityPlayerIsFreeAimingAt(player)
			if exist then
				local pos = GetEntityCoords(obj)
				local model = GetEntityModel(obj)

				print("{pos = vector3("..pos.x..", "..pos.y..", "..pos.z.."), model = "..model.."},")
			end
		end
...
RegisterNetEvent("vrp-doors:sync", function(doorId, state)
    if type(doorId) == "table" then
        allDoors = doorId
    else
        allDoors[doorId].isLocked = state
    end
end)

Citizen.CreateThread(function()
    local pedPos = vector3(0, 0, 0)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            for doorId in pairs(allDoors) do
                Citizen.Wait(10)
                local dst = #(allDoors[doorId].pos - pedPos)
                if dst <= 30.0 then
                    nearDoors[doorId] = {dst = dst}
                end
            end
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            for doorId in pairs(nearDoors) do
                if nearDoors[doorId] then
                    if not nearDoors[doorId].obj then
                        nearDoors[doorId].obj = GetClosestObjectOfType(allDoors[doorId].pos, 10.0, allDoors[doorId].model, false, false,
                            false)
                    end
                    local obj = nearDoors[doorId].obj
                    if obj ~= 0 then

                        local locked, heading = GetStateOfClosestDoorOfType(allDoors[doorId].model, allDoors[doorId].pos.x, allDoors[doorId].pos.y, allDoors[doorId].pos.z)
                        if allDoors[doorId].isLocked then
                            if heading > -0.02 and heading < 0.02 then -- thx fpt
                                FreezeEntityPosition(obj, true)
                            end
                            if not locked then
                                SetStateOfClosestDoorOfType(allDoors[doorId].model, allDoors[doorId].pos.x, allDoors[doorId].pos.y,allDoors[doorId].pos.z, true, 0.0)
                            end
                        else
                            FreezeEntityPosition(obj, false)
                            if locked then
                                SetStateOfClosestDoorOfType(allDoors[doorId].model, allDoors[doorId].pos.x, allDoors[doorId].pos.y,allDoors[doorId].pos.z, false, heading)
                            end
                        end
                    end
                    Citizen.Wait(10)
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if IsControlJustPressed(0, 38) then
                ...
                for doorId, data in pairs(nearDoors or {}) do
                    if data.obj then
                        if #(allDoors[doorId].pos - pedPos) <= 2.0 then
                            if not allDoors[doorId].faction or LocalPlayer.state.faction == allDoors[doorId].faction then
                                table.insert(anyDoor, doorId)
                                TaskTurnPedToFaceEntity(PlayerPedId(), data.obj, 100)
                            end
                        end
                ...
                if next(anyDoor) then
                    ...
                    local durr = GetAnimDuration("mp_arresting", "a_uncuff") * 550
                    TaskPlayAnimAdvanced(ped, "mp_arresting", "a_uncuff", pedPos, GetEntityRotation(ped), 2.0, 1.0, durr, 0, 0.05)
                    Citizen.Wait(durr)
                    ClearPedTasks(ped)

                    local lockStatus = -1
                    for _, doorId in pairs(anyDoor) do
                        if lockStatus == -1 then
                            lockStatus = not allDoors[doorId].isLocked
                        end

                        TriggerServerEvent("vrp-doors:toggle", doorId, lockStatus)
                    end
                end
            end
            Citizen.Wait(1)
