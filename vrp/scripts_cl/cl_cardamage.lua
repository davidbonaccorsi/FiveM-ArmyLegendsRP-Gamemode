local cfg = {
	deformationMultiplier = 4,					-- How much should the vehicle visually deform from a collision. Range 0.0 to 10.0 Where 0.0 is no deformation and 10.0 is 10x deformation. -1 = Don't touch. Visual damage does not sync well to other players.
	...
	damageFactorEngine = 30,					-- Sane values are 1 to 100. Higher values means more damage to vehicle. A good starting point is 10
	damageFactorBody = 30,					-- Sane values are 1 to 100. Higher values means more damage to vehicle. A good starting point is 10
	damageFactorPetrolTank = 70,				-- Sane values are 1 to 200. Higher values means more damage to vehicle. A good starting point is 64
	...
	cascadingFailureSpeedFactor = 30,			-- Sane values are 1 to 100. When vehicle health drops below a certain point, cascading failure sets in, and the health drops rapidly until the vehicle dies. Higher values means faster failure. A good starting point is 8
	...
	engineSafeGuard = 70.0,					-- Final failure value. Set it too high, and the vehicle won't smoke when disabled. Set too low, and the car will catch fire from a single bullet to the engine. At health 100 a typical car can take 3-4 bullets to the engine before catching fire.
...
local function isPedDrivingAVehicle()
	vehicle = GetVehiclePedIsIn(tempPed, false)
	if IsPedInAnyVehicle(tempPed, false) then
		if GetPedInVehicleSeat(vehicle, -1) == tempPed then
	...
	while (playerVehicle == veh) do
		...
		if isPedDrivingAVehicle() then
			...
			if pedInSameVehicleLast == true then
				...
				lastVehicle = GetVehiclePedIsIn(tempPed, true)				
