...
local delay_bcast_timer = 2000

local lastVehicleDataSent = {}
...
Citizen.CreateThread(function()
	while true do
			...
			--	SetPlayerCanDoDriveBy(PlayerId(), false)
				...
				while GetPedInVehicleSeat(veh, -1) == playerped do
					Wait(1)
					...
					if cls ~= 14 and cls ~= 15 and cls ~= 16 and cls ~= 21 then
					
						----- CONTROLS -----
						if not IsPauseMenuActive() then
						
							-- Avarii
              if (IsDisabledControlPressed(0, 84) and IsDisabledControlPressed(0, 83)) then -- INPUT_FRONTEND_CANCEL / Backspace
              ...
              -- IND L
              elseif IsDisabledControlJustReleased(0, 84) then -- INPUT_VEH_PREV_RADIO_TRACK
                  local cstate = state_indic[veh]
