...
$(window).ready(function() {
    resetUI();
    $.post('https://vrp/uiReady', {});
});

window.addEventListener('message', (event) => {
    let tempData = event.data;
    if(tempData.action == "draw"){
        $('.tunning-container').fadeIn();
        $("#drawmarker-container").css("left", "1%");
    }else if(tempData.action == "undraw"){
        $('.tunning-container').fadeOut();
        $("#drawmarker-container").css("left", "-50%");
    }
...
const tunningOnKey = (event) => {
    if (event.which == 9 && tunningActive) {
        return false;
    } else if (event.which == 39 || event.which == 68  && tunningActive) { // left (prev)
    ...
    } else if (event.which == 37 || event.which == 65  && tunningActive) { // right (next)
        event.preventDefault();
    ...
    } else if (event.which == 71  && tunningActive) { // g
        event.preventDefault();

        userDetailCardToggle = !userDetailCardToggle;
        fadeInDetailCard();
        fadeOutDetailCard();
    } else if (event.which == 38 || event.which == 87  && tunningActive) { // up (next)
        event.preventDefault();

        if ($('#colorPicker-holder').css("opacity") >= 0.5) {
            menuColorPickerGoto(0, -1)
        }
    } else if (event.which == 40 || event.which == 83  && tunningActive) { // down (next)
        event.preventDefault();

        if ($('#colorPicker-holder').css("opacity") >= 0.5) {
            menuColorPickerGoto(0, 1)
        }
    } else if (event.which == 8 && tunningActive) { // backspace
        event.preventDefault();

        $.post('https://vrp/handle', JSON.stringify({
            type: 'update',
            what: 'menu',
            user: 'backspace',
    ...
    } else if (event.which == 13  && tunningActive) { // enter
        event.preventDefault();
    ...
    } else if (event.which == 27  && tunningActive) { // esc
        event.preventDefault();
...
window.addEventListener("keydown", tunningOnKey)

function resetUI() {
    $('.tunning-container').hide();
    $('#buttonHelpers').hide();
    ...
    $('#options_container').stop(true, true).animate({height: '0px'}, 300, function() {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (data[i].price >= 0) {
                ...
                price = '<div class="options_container_option_price"><img src="https://cdn.armylegends.ro/tunning/check.png" style="width:20px;height:20px;"></div>';
            }
...
function playSound(soundName, volume) {
    ...
    audioElement.setAttribute('src', '../public/sounds/' + soundName + '.ogg');
