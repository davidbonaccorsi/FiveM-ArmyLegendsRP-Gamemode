local Config = module("cfg/tunning")
...
RegisterNetEvent("vrp-custom:setMoney", function(money)
    bani = money
end)

RegisterNetEvent('vrp-customs:openAdmin', function()
    isOpenByAdmin = true
    openUI()
end)
...
CreateThread(
    function()
        for i = 1, #Config.TunningPositions, 1 do
            local tempPos = Config.TunningPositions[i]
            if (tempPos.hasBlip == nil or tempPos.hasBlip == true) then
                addBlip(tempPos.pos, 72, 0, "Tunning")
            end
        end
        ...
        while (true) do
            ...
            if (not uiOpen) then
                if
                    ((playerVeh ~= 0) and (DoesEntityExist(playerVeh)) and
                        (GetPedInVehicleSeat(playerVeh, -1) == playerPed))
                 then
                    waitTime = 100
                    ...
                    for i = 1, #Config.TunningPositions, 1 do
                        ...
                        local tempActionDist = tempPos.actionDistance or Config.TunningActionDistance
                        ...
                        if (tempDist <= tempActionDist) then
                            ...
                            if not keyShown then
                                keyShown = true
                                TriggerEvent("vrp-hud:showBind", {key = "E", text = "Modifica-ti vehiculul"})
                            end
                            if (IsControlJustReleased(0, Config.TunningKeys.action.key)) then
                ...
                if (customConfigPosIndex) then
                    local tempPos = Config.TunningPositions[customConfigPosIndex]
...
function updateCash()
    TriggerServerEvent("vrp-custom:getMoney")
    Citizen.Wait(60)
    SendNUIMessage(
        {
            type = "update",
...
function openUI()
    ...
    if bani < 0 then
        tvRP.notify("Nu poti modifica masina deoarece ai restante", 'error')
        return
    end
    ...
    if GetEntityModel(playerVeh) == deluxoModel then
        TriggerEvent("vrp-hud:notify", "Nu poti modifica Deluxo !", "error")
        return
    end
...
function closeUI(sendToUI, resetVehToDefault)
    ...
    if (sendToUI == 1) then
        ...
        local vehicle = tvRP.getNearestOwnedVehicle(5.0)
    
        if vehicle then
            TriggerServerEvent("vrp-garages:saveStates", {[vehicle] = tvRP.getVehicleState(vehicle)})
        end
    end
...
function updateMenu(menuId)
    if (menuId == nil or Config.TunningMenus [menuId] == nil) then
        return
    end

    local menu = clearMenu(Config.TunningMenus [menuId])
        ...
        elseif (data.type == "update") then
            if (data.what == "menu") then
                if (data.user == "hover") then
                    ...
                    local menu = Config.TunningMenus [data.menuId]
                ...
                elseif (data.user == "enter") then
                    ...
                    if ((not isOpenByAdmin) and menuOption and (menuOption.price or menuOption.label == "Stoc")) then
                        if (menuOption.price == -1) then
                            if menuOption.label ~= "Stoc" then
                                canBuyMod = false
                            end
                        elseif (menuOption.price > 0) then
                            ...
                            if (tonumber(bani) >= tonumber(tempPrice)) then
                                canBuyMod = true

                                if (tempPrice > 0) then
                                    local bulletproof = false
                                    if data.menuId == "mod_bulletproof" then
                                        bulletproof = true
                                    end
                                    TriggerServerEvent("vrp-custom:removeMoney", tempPrice, bulletproof)
                                end
                            else
                                canBuyMod = false
                            end
...
function optionsShouldShow(menu)
    ...
    for i = 1, #menu.options, 1 do
        ...
        if (shouldShow and menu.options[i].openSubMenu ~= nil) then
            local subMenu = Config.TunningMenus [menu.options[i].openSubMenu]
            local tempShouldShow = false
            for i = 1, #subMenu.options, 1 do
                if (subMenu.options[i].modType ~= nil) then
                    if
                        (GetNumVehicleModData(customVehicle, subMenu.options[i].modType) >= 0 or
                            subMenu.options[i].openSubMenu ~= nil)
                     then
                        tempShouldShow = true
                        break
                    end
...
function createMenu(menuId, menuOption)
    ...
    if (menuOption.customType == "color" or menuOption.customType == "customColor") then
        Config.TunningMenus [newMenuId].title = ""
        return
    end
    ...
    for i = startIndex, GetNumVehicleModData(customVehicle, menuOption.modType), 1 do
        ...
        table.insert(
            Config.TunningMenus [newMenuId].options,
            {
                label = tempLabel,
                img = menuOption.img,
                price = tempPrice,
        ...
        if (menuOption.modType == 11 or menuOption.modType == 18) then
            local tempOption = Config.TunningMenus [newMenuId].options[#Config.TunningMenus [newMenuId].options]
        ...
        elseif (menuOption.modType == "extras") then
            ...
            tempOption.onSelect = function()
                ...
                Config.TunningMenus ["extras_on_off"] = {
                    title = "EXTRA " .. tostring(i + 1),
                    options = {
                        {
                            label = "OFF",
                            img = "assets/icons/minus.png",
                            price = tempPrice,
                            onHover = function()
                            ...
                            onSelect = function()
                                ...
                                Config.TunningMenus ["extras_on_off"].options[1].price = -1
                                Config.TunningMenus ["extras_on_off"].options[2].price = tempPrice
                        ...
                        },
                        {
                            ...
                            onSelect = function()
                                ...
                                Config.TunningMenus ["extras_on_off"].options[1].price = tempPrice
                                Config.TunningMenus ["extras_on_off"].options[2].price = -1
                ...
                if (isTempExtraOn == 0) then
                    Config.TunningMenus ["extras_on_off"].options[2].price = -1
                    Config.TunningMenus ["extras_on_off"].defaultOption = 1
                else
                    Config.TunningMenus ["extras_on_off"].options[1].price = -1
                end
