-- Labels 
local plateIndexLabel = {'Albastru pe Alb','Galben pe Negru','Galben pe Albastru','Albastru pe Alb','Albastru pe Alb','North Yankton'}
local windowTintLabel = {'Fara Folii','Folii 100%',  'Folii 80%',  'Folii 60%',  'Stoc',  'Limo',  'Verde'  }
local paintTypeLabel = {'Normala','Metalica',  'Perla',  'Mata',  'Metal',  'Crom'}
local modXenonLabel = {'Alb','Albastru','Albastru Electric','Verde','Verde Deschis','Galben','Auriu','Portocaliu','Rosu','Roz','Roz Aprins','Mov','Negru'}
...
function GetVehicleData(vehicle)
	...
	local tempData = {
		...
		fuelLevel         = math.ceil(GetVehicleFuelLevel(vehicle)),
		dirtLevel         = math.ceil(GetVehicleDirtLevel(vehicle)),
...
function GetNumVehicleModData(vehicle, modType)
	...
	elseif (modType == 'wheelColor') then
		return 0
	elseif (modType == 'bulletproof') then
		return 0
	elseif (modType == 'pearlescentColor') then
		return 0
	elseif (modType == 'tyreSmokeColor') then
...
function repairtVehicle(vehicle)
	if (not isOpenByAdmin) then
		...
		local fixingTime = ((targetEngineHealth - curEngineHealth) + (targetBodyHealth - curBodyHealth)) * 10
		
		local inFix = true
		Citizen.CreateThread(function()
			while inFix do
				drawText("Un mecanic iti repara masina...", 0.5, 0.85, 0.4, 255, 255, 255)
				Citizen.Wait(1)
			end
		end)

		Citizen.Wait(fixingTime)
		inFix = false
		...
		local cleaningTime = (curDirtLevel - targetDirtLevel) * 1000

		local inFix = true
		Citizen.CreateThread(function()
			while inFix do
				drawText("Un ucenic iti ~b~curata~w~ masina...", 0.5, 0.85, 0.4, 255, 255, 255)
				Citizen.Wait(1)
			end
		end)

		Citizen.Wait(cleaningTime)
		inFix = false
...
-- CAMERA
function moveCameraToBoneWithOffset(camera, vehicle, posOffset, rotOffset)
...
function moveToCameraToBoneSmoth(fromCamera, toCamera, vehicle, posOffset, rotOffset)
	local hasBone = moveCameraToBoneWithOffset(toCamera, vehicle, posOffset, rotOffset)
