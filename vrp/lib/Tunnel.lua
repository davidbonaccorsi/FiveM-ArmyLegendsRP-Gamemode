    ...
    if delay_data[2] > 0 then -- delay trigger
      SetTimeout(delay_data[2], function() 
        ...
        if type(callback) == "function" then -- ref callback if exists (become a request)
          ...
          TriggerClientEvent(iname..".callback__req",dest,key,args,identifier,rid)
        else -- regular trigger
          TriggerClientEvent(iname..".callback__req",dest,key,args,"",-1)
        end
      end)
    else -- no delay
      -- send request
...
function Tunnel.bindInterface(name,interface)
  -- receive request
  RegisterServerEvent(name..".callback__req")
    ...
    if type(f) == "function" then
      ...
      TUNNEL_DELAYED = function()
        ...
        return function(rets)
          ...
          if rid >= 0 then
            TriggerClientEvent(name.."."..identifier..".callback_res",source,rid,rets)
          end
        end
      end
...
function Tunnel.getInterface(name,identifier)
  ...
  -- receive response
  RegisterServerEvent(name.."."..identifier..".callback_res") -- rnd file
