...
var DudLength = 8;
var Sound = true;
var InfoText = "ROBCO INDUSTRIES (TM) TERMALINK PROTOCOL<br />GASESTE PAROLA CORECTA";
var Correct = "";
var Words = {};
var OutputLines = [];
var MaxAttempts = 4;
...
GetWords = function(length, count) {
	...
	Words = NewWords;
	Words = Shuffle(Words);
	Correct = Words[Math.floor(Math.random() * 6)];
  FillWordColumns();
}
...
{
	var AttemptString = AttemptsRemaining + " INCERCARI RAMASE: ";
...
TogglePower = function(win)
{
    ...
    $.post(`https://${GetParentResourceName()}/result`, JSON.stringify([win]));
	}
	else
	{
		Power = "on";
	...
	column.find(".character").click(function()
	{
		...
		if ($(this).hasClass("word"))
		{
			...
			else
			{
				if (Sound)
					$("#passbad")[0].play();
				UpdateOutput("Acces interzis");
				UpdateOutput( CompareWords(word, Correct) + "/" + Correct.length + " corecte." );
				AttemptsRemaining--;
				UpdateAttempts();
				if (AttemptsRemaining == 0)
					Failure();
...
Failure = function()
{
	...
	},
	{
		...
		complete : function()
		{
			$("#terminal").html("<div id='canvas'></div><div id='adminalert'><div class='character-hover alert-text'>TERMINAL BLOCAT</div><br />CONTACTEAZA ADMINISTRATORUL FIRMEI PENTRU DEBLOCARE</div></div>");
...
Success = function()
{
	...
	},
	{
		...
		complete : function()
		{
			$("#terminal").html("<div id='canvas'></div><div id='adminalert'><div id='msg' class='character-hover alert-text'>ACCES TERMINAL PERMIS</div><br /><div onClick=\"WinCondition(true);return false;\" id='proceed' class='alert-text'>APASA AICI PENTRU A INTRA</div></div>");
